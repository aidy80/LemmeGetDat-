	...
	for (int k = 0; k < numHands; k++) {
		...
		for (int i = 0; i < NUM_CARD_NUMBERS; i++) {
			if (numHist[i] == 2)
			{
				...
				{
					if (pokerhands[k].pair[1] > -1 && isHandCard(firstNum, secNum, pokerhands[k].pair[1])) 
					{
			...
			else if (numHist[i] == 4) 
			{
				...
				else if (pokerhands[k].pair[0] != -1 && isHandCard(firstNum, secNum, pokerhands[k].pair[0])) {
		...
		if (isQuads(pokerhands, k) || (!(pokerhands[k].flush || pokerhands[k].straight))) {
			int i;
			int numSeen = 0;
			for (i = NUM_CARD_NUMBERS - 1; numSeen < 5 && i > -1; i--)
			{
				if (numHist[i] == 1) {
...
char* getBestHands(Pool& pool, Hand* hands, unsigned char numHands)
{
	...
	PokerHand* pokerhands = new PokerHand[numHands];
...
std::vector<float> calcPreFlopEquity(Hand* hands, unsigned char numHands)
{
	...
	const unsigned int numTrials = (unsigned int)pow(52, 5);
	...
	
	{
		...
		for (unsigned int i = 0; i < numTrials; i++) {
			createRandomPoolCards(currPool);
			bool stillWinners = true;
			char* winners = getBestHands(currPool, hands, numHands);
			for (int j = 0; winners[j] != -1; j++) {
